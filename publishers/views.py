...
def ArticleCreationView(request,username,article_id):
   ...
   # publisher=Publisher.objects.get(account=user)
    ...
    formsets=inlineformset_factory(Article,Sections,form=SectionForm,extra=2)
    SectionForms=formsets(instance=article)
    form=ArticleModelForm(instance=article)
    if request.method=="POST":
        form=ArticleModelForm(data=request.POST,files=request.FILES,instance=article)
        SectionForms=formsets(data=request.POST,files=request.FILES,instance=article)
...
def publishView(request,article_id,article_slug):
    ...
    if request.method=="POST":
        ...
        if form.is_valid():
            ...
            article.title_slug=re.sub(r"(-transformedslugdjango)","",removeaddedslug)
            article.save()
            form.save()
            messages.add_message(request,messages.INFO,"Article published Successfully")
            return render(request,"publisher/articlePublisherView.html",
...
def editView(request,username,article_id):
    article= Article.objects.get(pk=article_id)
    formsets=inlineformset_factory(Article,Sections,form=SectionForm,extra=1)
    SectionForms=formsets(instance=article)
    form=ArticleModelForm(instance=article)
    if request.method=="POST":
...
def editPro(request,username,article_id):
    formsets=inlineformset_factory(Article,Sections,form=SectionForm,extra=0)
    if request.method=="POST":
